...
Event::Event()
{
    name = "NULL";
    place = "NULL";
    sday = smonth = syear = eday = emonth = eyear = hour = minute = curday = curmonth = curyear = day = 0;
    done = false;
}
...
void Event::add_event()
{
    ...
    cin >> n->hour >> n->minute;
    cout << "Enter Event Rminder Time : " << endl;
    cin >> n->rhour >> n->rminute;
    cout << "------------------------- " << endl;
...
void Event::display_events(int month, int day, int year)
{
    ...
    for (list<Event>::iterator it = d_new.begin(); it != d_new.end(); it++)
    {
        ...
        cout << "Reminder Time : " << it->rhour << ":" << it->rminute << endl;
        if (it->Remianing_Time(month, day, year) > 0)
        {
            cout << "Number of Days between Current Date And Event Start Date : ";
            cout << it->Remianing_Time(month, day, year) << endl;
        }
        else
        {
    ...
    else
    {
        ...
        for (list<Event>::iterator it = d.begin(); it != d.end(); it++)
        {
            if (it->name == name)
            {
                ...
                cin >> event.rhour >> event.rminute;
                it->rhour = event.rhour; it->rminute = event.rminute;
...
void Event::search_event(int month, int day, int year) {
    ...
    else
    {
        ...
        cin >> eventname;
        cout << "\n";
        file >> name >> place >> sday >> smonth >> syear >> eday >> emonth >> eyear >> hour >> minute >> rhour >> rminute;
        while (!file.eof()) {
            if (eventname == name) {
                cout << "---------------------------------------------- \n";
                ...
                cout << "Remainder Time : " << rhour << ":" << rminute << endl;
                if (Remianing_Time(month, day, year) > 0)
                {
                    cout << "Number of Days between Current Date And Event Start Date : ";
                    cout << Remianing_Time(month, day, year) << endl;
                }
                else
                {
                    cout << "There is No Remainder Time Because The Event is Already Started \n";
...
long Event::Remianing_Time(int month, int day, int year)
{
...
void Event::tomorrow(int month, int day, int year)
{
    ...
    else
    {
        ...
        for (list<Event>::iterator it = d.begin(); it != d.end(); it++)
        {
            if (it->Remianing_Time(month, day, year) == 1)
            {
                cout << "---------------------------------------------- \n";
                cout << "Event Name : " << it->name << endl;
                cout << "Event Place : " << it->place << endl;
